  ...
  try {
    ...
    if (!provider.forkSession) {
      return NextResponse.json(
        { error: `${provider.displayName} does not support runtime session fork` },
        { status: 501 }
      );
    }

    const body = await request.json().catch(() => null);
    const result = await provider.forkSession(runtime, sessionId, {
      title: isRecord(body) ? readString(body.title) : null,
    });
