...
export async function GET(
  ...
  try {
    ...
    if (!provider.discoverToolsets) {
      return NextResponse.json(
        { error: `${provider.displayName} does not support runtime toolsets discovery` },
        { status: 501 }
      );
    }

    const toolsets = await provider.discoverToolsets(runtime);
    return NextResponse.json({ toolsets });
  } catch (err) {
