...
export default function ChatPage() {
  ...
  return (
    <div className="flex h-[calc(100dvh-3.5rem)] lg:h-dvh flex-col">
      ...
      {voiceMode === "agent" && (
        <div className="fixed inset-0 z-50 overflow-hidden bg-[radial-gradient(circle_at_top,rgba(99,183,170,0.16),transparent_24%),radial-gradient(circle_at_bottom,rgba(86,113,184,0.12),transparent_28%),linear-gradient(180deg,rgba(11,18,25,0.98),rgba(8,13,18,0.96))] text-slate-50">
          <div className="absolute inset-0 bg-[linear-gradient(rgba(148,163,184,0.06)_1px,transparent_1px),linear-gradient(90deg,rgba(148,163,184,0.05)_1px,transparent_1px)] bg-[size:44px_44px] opacity-20" />
          <div className={`relative flex h-full flex-col ${agentOverlayMode === "immersive" ? "justify-center" : ""}`}>
              ...
              <div className="mx-auto flex max-w-5xl items-start justify-between gap-4 py-3">
                <div className="min-w-0">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-[11px] uppercase tracking-[0.35em] text-slate-400">Agent Mode</div>
                ...
                <div className="flex shrink-0 items-center gap-2">
                  <button
                    ...
                    className="flex h-10 w-10 items-center justify-center rounded-full border border-white/12 bg-white/8 text-slate-300 transition hover:bg-white/12 hover:text-white"
                  >
                    {agentOverlayMode === "transcript" ? (
                  ...
                  <button
                    ...
                    className="flex shrink-0 items-center gap-2 rounded-full border border-white/12 bg-white/8 px-4 py-2 text-[11px] font-medium tracking-[0.24em] text-slate-300 transition hover:bg-white/12 hover:text-white"
                  >
            ...
            <div className={`px-4 pb-4 sm:px-6 ${agentOverlayMode === "immersive" ? "flex-1 flex items-center" : "shrink-0"}`}>
              <div className={`mx-auto rounded-[32px] border border-white/10 bg-black/20 px-4 py-4 shadow-[0_30px_90px_rgba(0,0,0,0.35)] backdrop-blur-xl sm:px-6 ${agentOverlayMode === "immersive" ? "w-full max-w-none border-0 bg-transparent px-0 py-0 shadow-none backdrop-blur-none" : "max-w-5xl"}`}>
            ...
            {agentOverlayMode === "transcript" ? (
              <div ref={agentScrollContainerRef} className="min-h-0 flex-1 overflow-y-auto px-4 pb-[max(env(safe-area-inset-bottom),1rem)] sm:px-6">
                <div className="mx-auto max-w-4xl space-y-4 pb-8">
                  ...
                  {isLoading && (
                    <div className="flex justify-center py-4">
                      <div className="flex items-center gap-1.5 rounded-full border border-white/10 bg-white/6 px-4 py-3">
                ...
                {showScrollButton && (
                  <button
                    onClick={scrollToBottom}
                    className="sticky bottom-4 left-1/2 z-10 flex -translate-x-1/2 items-center gap-1.5 rounded-full border border-white/12 bg-black/45 px-4 py-2 text-xs text-slate-200 shadow-lg backdrop-blur-sm transition-all hover:border-white/20 hover:text-white animate-fade-in"
                  >
                    <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 13.5 12 21m0 0-7.5-7.5M12 21V3" />
                    </svg>
