...
const statusColors: Record<string, string> = {
  ...
  completed: "text-neo bg-neo/10 border-neo/20",
      ...
      <div className="flex-1 space-y-6 p-4 sm:p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-lg border border-neo/20 bg-neo/10 px-4 py-2 font-mono text-xs tracking-wider text-neo transition-all duration-200 hover:bg-neo/20 hover:shadow-[0_0_15px_rgba(0,240,255,0.15)]"
          >
        ...
        {showCreate && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowCreate(false)}>
            <div className="glass-card w-full max-w-md space-y-4 p-6" onClick={(e) => e.stopPropagation()}>
              <h2 className="font-mono text-sm font-bold tracking-[0.15em] text-white/80">NEW PROJECT</h2>
              <div className="space-y-3">
                <div>
                  <label className="mb-1 block font-mono text-[10px] tracking-wider text-white/30">NAME</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="mb-1 block font-mono text-[10px] tracking-wider text-white/30">COLOR</label>
                  <div className="flex items-center gap-2">
                    <input
                      type="color"
                      value={newProject.color}
                      onChange={(e) => setNewProject({ ...newProject, color: e.target.value })}
                      className="h-8 w-8 rounded border border-white/[0.08] bg-white/[0.03] cursor-pointer"
                    />
                    <input
                      type="text"
                      value={newProject.color}
                      onChange={(e) => setNewProject({ ...newProject, color: e.target.value })}
                      className="flex-1 rounded-lg border border-white/[0.08] bg-white/[0.03] px-3 py-2 font-mono text-xs text-white/70 placeholder-white/20 outline-none focus:border-neo/30"
                      placeholder="#00f0ff"
                    />
                  </div>
                </div>
            ...
            <div className="glass-card w-full max-w-2xl space-y-4 p-6" onClick={(e) => e.stopPropagation()}>
              ...
              {editing ? (
                <div className="space-y-3">
                  ...
                  <div>
                    <label className="mb-1 block font-mono text-[10px] tracking-wider text-white/30">CONTEXT / BACKGROUND</label>
                    <p className="mb-2 font-mono text-[9px] text-white/20">Background, constraints, client info — agents read this before picking up tasks. Markdown supported.</p>
                    <textarea
                      value={editContext}
                  ...
                  <div>
                    <label className="mb-1 block font-mono text-[10px] tracking-wider text-white/30">DOCUMENTS / REFERENCES</label>
                    <div className="space-y-2">
                      {editDocs.map((doc, i) => (
                        <div key={i} className="flex items-center gap-2">
                          <input
                            ...
                            placeholder="Name..."
                            className="w-1/3 rounded-lg border border-white/[0.08] bg-white/[0.03] px-2 py-1.5 font-mono text-[11px] text-white/70 outline-none focus:border-neo/30"
                          />
                          <input
                            type="text"
                            value={doc.url}
                            ...
                            className="flex-1 rounded-lg border border-white/[0.08] bg-white/[0.03] px-2 py-1.5 font-mono text-[11px] text-white/70 outline-none focus:border-neo/30"
                          />
                          <button
                      ...
                      <button
                        ...
                        className="font-mono text-[10px] tracking-wider text-neo/60 transition-colors hover:text-neo"
                      >
                        + ADD DOCUMENT
                      </button>
                    </div>
                ...
                <div className="space-y-4">
                  ...
                  {selectedProject.context && (
                    <div className="rounded-lg border border-neo/15 bg-neo/[0.03] p-4">
                      <div className="mb-2 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <svg className="h-3.5 w-3.5 text-neo/50" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M12 6.042A8.967 8.967 0 0 0 6 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 0 1 6 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 0 1 6-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0 0 18 18a8.967 8.967 0 0 0-6 2.292m0-14.25v14.25" />
                          </svg>
                          <span className="font-mono text-[10px] tracking-[0.15em] text-neo/60">
                            CONTEXT / BACKGROUND
                          </span>
                        </div>
                      ...
                      <div className="prose-invert max-h-48 overflow-y-auto font-mono text-[11px] leading-relaxed text-white/50 [&_a]:text-neo/70 [&_a]:underline [&_code]:rounded [&_code]:bg-white/[0.06] [&_code]:px-1 [&_code]:text-neo/60 [&_h1]:mb-2 [&_h1]:text-xs [&_h1]:font-bold [&_h1]:text-white/60 [&_h2]:mb-2 [&_h2]:text-[11px] [&_h2]:font-bold [&_h2]:text-white/60 [&_h3]:mb-1 [&_h3]:text-[11px] [&_h3]:font-bold [&_h3]:text-white/50 [&_li]:ml-4 [&_ol]:list-decimal [&_p]:mb-2 [&_pre]:mb-2 [&_pre]:rounded [&_pre]:bg-white/[0.04] [&_pre]:p-2 [&_ul]:list-disc">
                  ...
                  <div className="flex flex-wrap gap-4 border-t border-white/[0.06] pt-4">
                    <div>
                      <span className="font-mono text-[9px] tracking-wider text-white/20">PROGRESS</span>
                      <div className="mt-1 flex items-center gap-2">
                        ...
                        <span className="font-mono text-[10px] text-neo/60">
                          {getProgress(selectedProject.id)}%
                        </span>
                      </div>
                    </div>
                    <div>
                      <span className="font-mono text-[9px] tracking-wider text-white/20">TASKS</span>
                    ...
                    <div>
                      <span className="font-mono text-[9px] tracking-wider text-white/20">UPDATED</span>
                      <div className="mt-1 font-mono text-[11px] text-white/50">
                  ...
                  {selectedProject.documents && selectedProject.documents.length > 0 && (
                    <div className="border-t border-neo/20 pt-4">
                      <div className="mb-3 flex items-center gap-2 rounded-lg border border-neo/20 bg-neo/5 px-3 py-2">
                        <svg className="h-4 w-4 shrink-0 text-neo/60" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M11.25 11.25l.041-.02a.75.75 0 0 1 1.063.852l-.708 2.836a.75.75 0 0 0 1.063.853l.041-.021M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Zm-9-3.75h.008v.008H12V8.25Z" />
                        </svg>
                        <span className="font-mono text-[10px] tracking-wider text-neo/70">
                          READ BEFORE PICKING UP TASKS
                        </span>
                      </div>
                      <h3 className="mb-3 font-mono text-[10px] tracking-[0.2em] text-white/30">
                      ...
                      <div className="space-y-2">
                        {selectedProject.documents.map((doc, i) => (
                          <a
                            ...
                            className="flex items-center gap-2 rounded-lg border border-neo/10 bg-neo/[0.04] px-3 py-2 transition-colors hover:border-neo/30 hover:bg-neo/10"
                          >
                            <svg className="h-4 w-4 shrink-0 text-neo/50" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                              <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
                            </svg>
                            <span className="flex-1 truncate font-mono text-[11px] text-white/70">{doc.name}</span>
                            <span className="shrink-0 font-mono text-[9px] text-neo/40">{doc.url.startsWith("http") ? "↗ LINK" : "→ FILE"}</span>
                          </a>
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="flex items-center justify-between border-t border-white/[0.04] pt-3">
                    <div className="flex items-center gap-2">
                      ...
                      {project.documents && project.documents.length > 0 && (
                        <span className="rounded border border-neo/20 px-1.5 py-0.5 font-mono text-[8px] tracking-wider text-neo/50">
                    ...
                    <span className="font-mono text-[9px] text-white/25 transition-colors group-hover:text-neo">
                      VIEW DETAILS →
                    </span>
                  </div>
                </div>
